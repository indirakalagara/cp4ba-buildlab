locals {
    ldap_host_ip     = var.ldap_host_ip != "" && var.ldap_host_ip != null ? var.ldap_host_ip : ""
    ldap_admin_name       = var.ldap_admin_name != "" && var.ldap_admin_name != null ? var.ldap_admin_name : ""
    ldap_admin_password    = var.ldap_admin_password != "" && var.ldap_admin_password != null ? var.ldap_admin_password : ""
    db2_host_address = var.db2_host_address != "" && var.db2_host_address != null ? var.db2_host_address : ""
    db2_ports     = var.db2_ports != "" && var.db2_ports != null ? var.db2_ports : ""
    db2_admin_username   = var.db2_admin_username != "" && var.db2_admin_username != null ? var.db2_admin_username : ""
    db2_user         = var.db2_user != "" && var.db2_user != null ? var.db2_user : ""
    db2_admin_user_password    = var.db2_admin_user_password != "" && var.db2_admin_user_password != null ? var.db2_admin_user_password : ""
  ...
  cp4ba_deployment_file_content             = templatefile("${path.module}/templates/cp4ba_deployment.yaml.tmpl", {
    ...
    ldap_host_ip     = local.ldap_host_ip 
    db2_host_address = local.db2_host_address
    db2_host_port     = local.db2_ports
    ingress_subdomain = var.ingress_subdomain,
    storage_class_rwo = var.storage_class_rwo,
    storage_class = var.storage_class
  })
  secrets_content    = templatefile("${path.module}/templates/secrets.yaml.tmpl", {
    ldap_admin       = local.ldap_admin_name
    ldap_password    = local.ldap_admin_password
    db2_admin        = local.db2_admin_username
    db2_user         = local.db2_ports
    db2_password     = local.db2_admin_user_password
